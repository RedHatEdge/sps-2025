...
Create a new catalog referring to your private registry using the YAML files generated by the oc-mirror plugin output, accessible from IPC4.

```bash
$ oc debug -n oc-mirror pods/run-oc-mirror-5xcfd 
Starting pod/run-oc-mirror-5xcfd-debug-j5zk5 ...
Pod IP: 10.42.0.36
If you don't see a command prompt, try pressing enter.
sh-5.1$ ls
oc-mirror  oc-mirror-workspace	oc-mirror.tar.gz
sh-5.1$ cd oc-mirror-workspace/
sh-5.1$ ls
publish  results-1773163370  results-1773218678
sh-5.1$ cd results-1773218678/
catalogSource-cs-gitea-catalog.yaml          charts/                                      mapping.txt                                  
catalogSource-cs-redhat-operator-index.yaml  imageContentSourcePolicy.yaml                release-signatures/                          
sh-5.1$ cat results-1773218678/catalogSource-cs-redhat-operator-index.yaml 
...
spec:
  image: registry.oc-mirror.svc.cluster.local:5000/redhat/redhat-operator-index:v4.20
  sourceType: grpc
```

Now you can create the ICSP file on the cluster pointing to the mirror registry (remember to change the internal URL `registry.oc-mirror.svc.cluster.local:5000` with `registry-oc-mirror.apps.ipc4.sps2025.com`):

```bash
$ oc apply -f imageContentSourcePolicy.yaml
```

You should now be able to access the catalog and see only the mirrored operators:
...
> On Windows make sure you are on the same network subnet as the devices to look for them with this discovery tool. You might have to setup the IP addresses manually

![alt text](image-6.png)

There are 2 identical IO Modules installed.

You can use the credentials found in [here](https://github.com/RedHatEdge/sps-2025-systems) to access the web interface to configure the IO Module.  

![alt text](image-5.png)

![alt text](image-11.png)

Inside the web console you can also change the IP address of the IO Module.  
...
To find the drive on the network install the [step-servo-tuner](workloads/ETH-IP-StepServoDrive/Step-Servo-Quick-Tuner-Setup-3.0.21.1120(AMP).exe)

> On Windows make sure you are on the same network subnet as the devices to look for them with this discovery tool. You might have to setup the IP addresses manually  

On connecting the Drive you will be asked if you want to download the current configuration  

![alt text](image-7.png)
![alt text](image-8.png)
You can change the IP address in the following interface, by first selecting which network interface to use for discovering:    
...
The resulting payload sent to `defect_detection/results` topic is in this format:  
...
> For the full picture of how the CODESYS project itself maps to the physical demo stand's field devices and network, see [workloads/Codesys/CODESYS-Workflow.md](workloads/Codesys/CODESYS-Workflow.md).
